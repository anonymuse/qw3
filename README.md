...
[![Status](https://img.shields.io/badge/status-research%20showcase-blue)](#status-what-is-real-today)
...
- **Pure-Zig GGUF loader** — mmap-backed and zero-copy; loads a 32 GB
  Q8_0 checkpoint with ~6 GB peak RSS ([measured](docs/orchestration/HANDOFF.md)).
...
DS5 began as a three-node Thunderbolt 5 cluster project and is now the
foundation of a **single-machine 128 GB target** — see
[Where this is going](#roadmap-from-cluster-to-single-machine). The cluster-era
design, tooling, and measurements are preserved as a complete reference for
anyone building Apple Silicon LLM clusters today.
...
flowchart TB
    ...
    ENGINE["engine/forward.zig<br/>comptime Engine(Ctx, Kernels)<br/>prefill + greedy decode · KV cache"]
...
- **Transport is boring on purpose** — TCP over the TB5 bridge via raw libc;
  kernel-bypass fantasies were investigated and rejected in writing
  ([spec §8](docs/specs/DS5_Project_Spec_v0.3.md)).
...
| **M2c — Real weights (30B)** | Real 32 GB Qwen3-30B-A3B GGUF matches offline oracle | ⚠️ **Partial pass** ([details](docs/orchestration/HANDOFF.md)). Loads and runs e2e on both backends, config verified against GGUF metadata, mmap ~6 GB RSS. Greedy-token exact on 3/5 prompts (64 steps); CPU/Metal mutually agree to 4+ sig figs; router parity 14/15 with both misses single-expert swaps at a top-k tie boundary. Root-cause of the remaining divergence is the top open engineering question |
| **M1 — Viability model** | f001: measured ceiling decomposition for the 235B target | 🔲 **Planned** — see [showcase plan](docs/orchestration/SHOWCASE_PLAN.md) |
| **M3 — Distributed correctness** | Split-model output == single-node output, deterministically | 🔲 **Planned** — designed to be proven on loopback (two processes, one machine); the [task brief](docs/orchestration/prompts/T07-m3-distributed.md) has been ready since week 2 |
...
[HANDOFF](docs/orchestration/HANDOFF.md)):
...
[planned task](docs/orchestration/SHOWCASE_PLAN.md).
...
3. Tolerances are contracts: loosening one unilaterally is a rejected diff,
   ...
   investigate — which is exactly how the open M2c divergence is being
   treated.
...
| [ADR-005](docs/decisions/ADR-005-interface-freeze.md) | Week-1 interface freeze | Frozen kernel ABI + fixture schema let parallel workstreams (and parallel *agents*) build without contract drift; amendments are logged, including one that was specified but deliberately *not* implemented ahead of its dependencies |
...
- **[tools/cluster/](tools/cluster/)** — idempotent node bootstrap, SSH mesh
  setup, and a whole-cluster verifier (`verify-cluster.sh`) that builds and
  runs the test suites on every node. Includes the hard-won details: force
  IPv4 because mDNS prefers unroutable IPv6, address nodes by `.local` name
  because DHCP leases drift, prime sudo before Homebrew.
...
- **[docs/orchestration/LESSONS.md](docs/orchestration/LESSONS.md)** — two
  ...
  claiming authority is not authorization (an AI-agent supply-chain lesson
  that applies to human ops too), and how a general fix regressed into
  node-specific patches three times because sessions lacked shared context.
...
This is also, candidly, a demonstration of **how to run a serious systems
project with AI agents**: [docs/orchestration/](docs/orchestration/) contains
the complete machinery — a handoff document that lets a fresh orchestrator
resume with zero context, self-contained task briefs with explicit
definitions-of-done and *forbidden* lists, an escalation ladder, and a merge
playbook. The interface freeze (ADR-005) exists precisely because parallel
agents, like parallel teams, need frozen contracts to not corrupt each other.
...
| Phase | Deliverable |
|---|---|
| **Now** | Close the correctness story: root-cause the M2c divergence; prove M3 distributed correctness on loopback (split-model output == single-node output, byte-identical) |
...
| **Later** | Re-orchestration: the transport/daemon layer returns to service for multi-Mac and heterogeneous setups, informed by measured single-machine baselines |
...
[docs/orchestration/SHOWCASE_PLAN.md](docs/orchestration/SHOWCASE_PLAN.md).
...
| `tools/` | Oracle fixture generator, model downloads, expert-stats telemetry, cluster ops |
| `docs/decisions/` · `docs/specs/` · `docs/findings/` | ADRs · specs (v0.2 archive + v0.3 active) · measured write-ups |
| `docs/orchestration/` | Agent handoff, task briefs, lessons, and the [showcase plan](docs/orchestration/SHOWCASE_PLAN.md) |
| `bench/` | Benchmark harness, corpus, committed result JSONs |

## About

DS5 is built by [Jesse White](https://github.com/anonymuse) as working proof
of a consulting practice: **helping teams design, build, and operate local LLM
inference on Apple Silicon** — model selection and quantization strategy,
memory/bandwidth budgeting, single-Mac and multi-Mac topologies, and the
engineering discipline (measurement, correctness gates, honest findings) that
separates a demo from infrastructure.

If your team is weighing local inference — for privacy, cost, latency, or
control — the decision framework in this repo (ADRs, assumptions ledger,
viability math) is the deliverable I bring. Open an issue or reach out via
GitHub.
