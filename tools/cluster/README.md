...
1. Fetch `bootstrap.sh` onto the Mac. **Do not `git clone` yet** — git isn't on a
   ...
   to `~/Code/qw3` once git is available — no manual clone needed.
...
5. Copy the printed **NODE FACTS** block. The facts are also saved to
   `~/Code/ds5-cluster/node-facts-<hostname>.txt` — you can **AirDrop** this file
   to Node A, or copy the printed block. Passing `--share` uploads them to a public
   paste URL (note: the URL is PUBLIC and contains the SSH public key + LAN IP,
   but no secrets). You'll paste all three into the orchestrator chat.
...
The repo is already at `~/Code/qw3` from bootstrap — just `cd ~/Code/qw3 && claude` to open the agent.
